        ...
        if($isHave){
            ...
            if(isset($data['idCardfront'])){
                ...
                if($data['idCardfront']){
                    ...
                    $type = 'firstLast';
                    // 判断付款方式，如果是全款则直接为初审通过
                    $down = Buy::where('phone',$data['phone'])->value('down');

                    $updateStatus = ['status'=>0];
                    if($down == "全款"){
                        $updateStatus = ['status'=>2,'firstTrial'=>Admin::where('user',"admin")->value('id')];
                    }
                    // 如果已经有了复审人员，再一次提交的话说明是复审未通过，那么此时再一次提交则认定为初审通过
                    $finalTrial =  Buy::where('phone',$data['phone'])->value('finalTrial');
                    if($finalTrial){
                        if($down == "全款"){
                    ...
                    Buy::where('id',$isHave)->update($updateStatus);
                }
            }else{
