...
//        $this->sms = $sms;
    ...
    public function send(Request $request){
        ...
        $message = $alisms->sms($all['phone'],SIGNNAME,MESSAGESEND,$code);
...
//        return response()->json(['code'=>403,'msg'=>'上传错误']);
    ...
    public function config(Request $request){
        ...
        $data = Config::where('id',1)
            ->select('project_name','img','registration_notes'
                ,'frozen','notice','updated_end',"end_at",
                "font_color","background_color","background")
            ->first();
        $data->end_msg  = "";
        $data->isBuy        = ISBUY;
    ...
    public function indexConfig(){
        ...
        $add        = 1;
        $floorRead  = FLOORREAD;
        $sell       = time() > strtotime($config->sell) ? 1 : 0;
        $result     = RESULT;
//        if(time() < strtotime($config->start_at)){
//            $add = 0;
//        }
    ...
    public function result(Request $request){
        ...
        $status                 = Message::check($get);
        if($status == 200){
            if(!Buy::where(['phone'=>$get['phone']])->value('id')){
                return response()->json(['code'=>403,'msg'=>'无效的手机号']);
            }
            $data               = Result::where(['phone'=>$get['phone']])->where('result',"<",2)->first();
            $idCard             = Buy::where('phone',$get['phone'])->first();

            if($data){
                $data->result   = $data->result == 0 ? "未中签" : "已中签";
                $name           = Buy::where(['phone'=>$get['phone']])->value('name');
//                dd(['code'=>200,'msg'=>'查询成功','result'=>$data->result,'name'=>$name,'idCard'=>$idCard->idCard,'sign'=>$idCard->md5]);
                return response()->json(['code'=>200,'msg'=>'查询成功','result'=>$data->result,'name'=>$name,'idCard'=>$idCard->idCard,'sign'=>$idCard->md5]);
            }else{
    ...
    public function loginDirect(Request $request){
        $md5 = $request->input('loginSign');
        $info = Buy::where('md5',$md5)->first();
        if(!$md5){
            return response()->json(['code'=>403,'msg'=>"登录异常请检查"]);
        }
        if(!$info){
            return response()->json(['code'=>403,'msg'=>"登录异常请检查"]);
        }else{
            if(in_array($info->status,[0,1,2,3,4,7,5])){
                $userStatus = "跳";
            }else{
                $userStatus  = "不跳" ;
            }
            return response()->json(['code'=>200,'msg'=>"登录成功",'status'=>$userStatus]);
        }
    }
