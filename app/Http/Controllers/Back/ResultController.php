...
class ResultController extends Controller
{
    public function excelLoad(Request $request){
        ...
        Excel::load($file, function($reader) use ($file){
            ...
            foreach ($data as $k=>$v){
                $isHave = Buy::where('registration',NAME.$v[1])->first();
                if($isHave){
                    $create['phone']        = $isHave->phone;
                    $create['registration'] = NAME.$v[1];
                    $create['getHouse']     = $v[0];
                    $create['name']         = $isHave->name;
...
//                        ]);
    ...
    public function resultPage1(){
        $title = "resultPage";
        return view('Back.result.test1',compact('title'));
    }
    /**
     * method: 获取身份证判断是否有结果
     * author: hongwenyang
