...
function callbackfun(gbase64,view,v,imgsrc,file,list){
	$.hideLoading();
	$.showLoading("开始上传..."); 
	updateimg(gbase64,view,v,imgsrc,file,list,view);//去上传
}
function updateimg(obj,node,v,imgsrc,file,list,view){
            console.log(node);
            console.log(v);
            console.log(imgsrc);
            $.hideLoading();
...
//								setlocalStorage(v+"length",vlength*1+1);
		               ...
		               var for1=$("#"+node).parents("label").attr('for');
		               $("#"+node).parents("label").attr('for1',for1);
		               $("#"+node).parents("label").removeAttr('for');
		               $("#"+node).parents("label").addClass('labelfor');
		               console.log(for1)
		               hidebg()
		               $.hideLoading();
	                   $.toast("上传成功!",500);
...
if($(".lastinfo").find('.trustpeople').length>0){  
...
function hidebg(){
	...
	if($(".page3").find(".childs").length>0){
		var buyothersbox=$(".childs").find(".child_img_box_child");
		for (var i = 0; i < buyothersbox.length; i++) {
			var name=$(".childs").find(".child_img_box_child").attr("name");
			var alt=$(".childs").find(".child_img_box_child").eq(i).find(".haschild"+(1+i)).attr("alt");
	        var src=$(".childs").find(".child_img_box_child").eq(i).find(".haschild"+(1+i)).attr("urlsrc");
	        if(alt==src||src==undefined){
				$(".btn").removeClass("btnok");
				return;
			}else{
				$(".childs").find(".child_img_box_child").eq(i).find(".haschild"+(1+i)).parent("label").removeClass("imgcolor");
			}
		}
	}
...
$.showLoading();
var c=support_canvas_text();
if(!c){
	$.alert('该浏览器不支持查看,请您在其他浏览器中打开,或者使用其他手机!');
   	return;
}else{
	console.log('支持canvas')
}
function setbigimg(){
	for (var i = 0; i < $(".imgfileboxcancel").length; i++) {
		 var for1=$(".imgfileboxcancel").eq(i).parent(".inputcon").find('label').attr('for');
		 var inputconkong=$(".imgfileboxcancel").eq(i).parent(".inputcon").find('label').find('.inputconkong').length
		 if(inputconkong==0){
		 	 $(".imgfileboxcancel").eq(i).parent(".inputcon").find('label').attr('for1',for1);
			 $(".imgfileboxcancel").eq(i).parent(".inputcon").find('label').removeAttr('for');
			 $(".imgfileboxcancel").eq(i).parent(".inputcon").find('label').addClass('labelfor');
		 }
...
console.log(deHeight);
$('.bigimgzoo').css({'min-height':deHeight});
...
$(".page3").on("click",'.labelfor',function(){
	var src=$(this).find(".show_img").attr('src');
	$(".bigimgzoo").show();
	$(".bigimgzoo .bigimgzooimg").attr('src',src)
	console.log(src);
})
$(".bigimgzoo").click(function(){
	$(".bigimgzoo").hide();
})
getdata1();
function getdata1(){
    ...
    success: function (obj) {
        ...
        if(obj.code == 200){
			 ...
			 if(obj.data.name==null){
			 	$.alert('请您先填写补充信息！',function(){
	                var v=Math.random();
	                window.location.href='index2.html?v='+v;
			    })
			 	return;
			 }
			 console.log(obj.data.down)
			 console.log(obj.data.pay)
			 if(obj.data.down=='全款'&&obj.data.pay=='一次性付款'){
			 	$(".lastinfo .trustpeople").remove();
			 }
			 ...
			 if(obj.data.child.length>0){
			 	$(".childs").show();
			 	var str='';
			     for (var i = 0; i < obj.data.child.length; i++) {
			     	str=str+'<div class="child_img_box_child child_img_box_child'+i+'"  name="'+obj.data.child[i].name+'">';
			     	str=str+'<div class="page3-header padding30"><span class="username">'+obj.data.child[i].name+'</span>户口本个人页<span class="page3-header-tips">(需上传清晰、完整、工整的照片)</span></div>'
			     	str=str+'<div class="imgfilecontair imgfilecontairinputcon  inputcon"><img class="imgfileboxcancel" src="img/closenum.png"/>';
			     	str=str+'<label for="childimg'+(i+1)+'"  class=""><img id="childimgimg'+(i+1)+'" class="show_img haschild'+(i+1)+'" alt="img/addimgicon.png" idCard="'+obj.data.child[i].idCard+'" src="img/addimgicon.png"/></label>';
			     	str=str+'<input type="file"     id="childimg'+(i+1)+'" class="inuputfile" accept="image/*"   haschild="0" onchange="javascript:setImagePreview('+"'childimg"+(i+1)+"','childimgimg"+(i+1)+"');"+'"  value="" /></div></div>';
			     }
			     str=str+'<div class="height40div"></div>';
			 	 $(".childszoo").html(str);
			 }else{
			 	$(".childs").remove();
			 }
...
//	return;
   ...
   if($(".lastinfo").find('.trustpeople').length>0){  
    ...
    if(obj.data.child_img.length>0){
    	for (var i = 0; i < obj.data.child_img.length; i++) {
    		$(".childszoo").find(".child_img_box_child").eq(i).find(".haschild"+(i+1)).attr("src",obj.data.child_img[i].accountBookpersonal);
    		$(".childszoo").find(".child_img_box_child").eq(i).find(".haschild"+(i+1)).attr("urlsrc",obj.data.child_img[i].accountBookpersonal);
    		$(".childszoo").find(".child_img_box_child").eq(i).find(".haschild"+(i+1)).parents(".inputcon").find(".imgfileboxcancel").show();
    	}
    }
    $("body").attr("status",obj.data.status);
    $.hideLoading();
...
//			
	...
	if($(".page3").find(".childs").length>0){
		var childsbox=$(".childs").find(".child_img_box_child");
		for (var i = 0; i < childsbox.length; i++) {
			var name=$(".childs").find(".child_img_box_child").eq(i).attr("name");
			var alt=$(".childs").find(".child_img_box_child").eq(i).find(".haschild"+(1+i)).attr("alt");
	        var src=$(".childs").find(".child_img_box_child").eq(i).find(".haschild"+(1+i)).attr("urlsrc");
	        if(alt==src||src==undefined){
				$(".childs").find(".child_img_box_child").eq(i).find(".haschild"+(1+i)).parent("label").addClass("imgcolor");
				$(".btn").removeClass("btnok");
				$.alert("未成年子女("+name+")的材料不完整！");
				return;
			}
		}
	}
...
var child_img=[];
var child_img_box_child=$(".childs").find(".child_img_box_child");
for (var i = 0; i < child_img_box_child.length; i++) {
	var alt=$(".haschild"+(i+1)).attr('alt');
	var src=$(".haschild"+(i+1)).attr('src');
	var idcard=$(".haschild"+(i+1)).attr('idcard');
	var data={};
	data.accountBookpersonal=src;
	data.idCard=idcard;
	child_img.push(data);
}
if(child_img==[]){
...
$(".page3").on("click",".imgfileboxcancel",function(){
		     var for1=$(this).parent(".inputcon").find('label').attr('for1');
		 	 $(this).parent(".inputcon").find('label').attr('for',for1);
			 $(this).parent(".inputcon").find('label').removeClass('labelfor');
...
//                  var imageSize = e.total;//图片大小
					...
					//火狐下，直接设img属性 
						if (window.navigator.userAgent.indexOf("Chrome") >= 1 || window.navigator.userAgent.indexOf("Safari") >= 1) { 
						     image.src = window.webkitURL.createObjectURL(source.files[0]); 
						}else{ 
						     image.src = window.URL.createObjectURL(source.files[0]); 
						} 
					}else{ 
						$("暂不支持您的浏览器，请更换其他浏览器！")
						return;
...
//					           }
					            ...
					            console.log('imageSize:'+imageDate.length/1024/1024+'M')
					            if(imageDate.length/1024/1024>1){
					            	$.hideLoading();
								    $.showLoading("压缩图片"); 
								    console.log('压缩图片')
						        	imageDate = canvas.toDataURL("image/jpeg", 0.7);
						        	newImage.src = imageDate;
						        	console.log('压缩后imageSize:'+imageDate.length/1024/1024+'M')
						        }
